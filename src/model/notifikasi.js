...
const createNewNotifikasi = (data) => {
    ...
    const SQL = `INSERT INTO notifikasi (id_pengguna, judul, pesan) VALUES (?, ?, ?)`;
...
const updateNotifikasi = (idNotifikasi, data) => {
    ...
    const SQL = `UPDATE notifikasi 
                 ...
                 WHERE id_notifikasi = ?`;
    return dbPool.execute(SQL, [judul ?? null, pesan ?? null, is_read ?? null, idNotifikasi]);
}
...
//Fungsi baru untuk menandai semua notifikasi menjadi "Sudah Dibaca" (is_read = 1)
const markAllAsReadByUser = (idPengguna) => {
    const SQL = `UPDATE notifikasi SET is_read = 1 WHERE id_pengguna = ? AND (is_read = 0 OR is_read IS NULL)`;
    return dbPool.execute(SQL, [idPengguna]);
};
